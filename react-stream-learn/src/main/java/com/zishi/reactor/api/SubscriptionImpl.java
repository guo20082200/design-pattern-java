...
public final class SubscriptionImpl<T> implements Subscription, Runnable {
    ...
    private void doSubscribe() {
        try {
            iterator = null; //elements.iterator();
            if (iterator == null) {
        ...
        if (!cancelled) {
            ...
            if (!hasElements) {
                ...
                Throwable t) { // As per rule 2.13, `onComplete` is not allowed to throw exceptions, so we do what we can, and log this.
    ...
    private void terminateDueTo(final Throwable t) {
        ...
        Throwable t2) { // If `onError` throws an exception, this is a spec violation according to rule 1.9, and all we can do is to log it.
